...
function Admin(props) {
  ...
  function update() {
    axios.post("http://localhost:8000/reference", { image, title, text, id }, {headers:{Authorization:`Bearer ${sessionStorage.getItem("token")}`}}).then((res) => {
      alert(res.data)
    }).catch((err) => {
        ...
        <div className='w-full bg-[#0F1035]/40 rounded-e-xl p-7'>
          ...
          <div className='flex flex-col w-4/5 mx-auto my-8'>
            ...
            <div className='flex flex-col group'>
              ...
              <img src={image} alt="Referencia kép" className='absolute left-0 top-0 group-hover:left-[--x] group-hover:top-[--y] h-40 rounded-lg z-10 opacity-0 group-hover:opacity-100 pointer-events-none' style={{"--x": x + "px", "--y": y + "px"}} />
            </div>
